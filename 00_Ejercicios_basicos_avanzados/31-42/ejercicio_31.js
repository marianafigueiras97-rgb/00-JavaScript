...
for (let i = 0; i < artists.length; i++) {
  const currentArtist = artists[i].name;
  
  for (let j = 0; j < 1; j++) {
    const directInfluence = artists[i].influences[0];
    console.log(`El artista ${currentArtist} fue influenciado por ${directInfluence}`);
  }
